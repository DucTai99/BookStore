                                    ...
                                    <div class="table-responsive" id="checkout-review-table-wrapper">
                                        <table class="data-table" id="checkout-review-table">
                                            ...
                                            <% ArrayList<Sach> listSach = (ArrayList<Sach>) request.getAttribute("listSach");
                                                ...
                                                %>
                                            <tr>
                                                <td><h3 class="product-name"><%=cart.gioHang.get(i).getTenSach()%></h3></td>
                                                <% if (cart.gioHang.get(i).getKhuyenMai() > 0) { %>
                                                <td><span class="cart-price"><span
                                                        class="check-price"><%=cart.gioHang.get(i).tienKhuyenMai()%>.000đ</span></span></td>
                                                <%} else {%>
                                                <td><span class="cart-price"><span
                                                        class="check-price"><%=cart.gioHang.get(i).getGia()%>.000đ</span></span></td>
                                                <%}%>
                                                <td><%=cart.gioHang.get(i).getSoLuongMua()%></td>
                                                <%  int tongTienSach = 0;
                                                    if (cart.gioHang.get(i).getKhuyenMai() > 0) {
                                                        tongTienSach = cart.gioHang.get(i).tienKhuyenMai()*cart.gioHang.get(i).getSoLuongMua();
                                                    } else {
                                                        tongTienSach = cart.gioHang.get(i).getGia()*cart.gioHang.get(i).getSoLuongMua();
                                                    }%>
                                                <!-- sub total starts here -->
                                                <td><span class="cart-price"><span
                                                        class="check-price"><%=tongTienSach%>.000đ</span></span></td>
                                            </tr>
                                         <%}%>
                                            </tbody>
                                            <tfoot>
                                            <tr>
                                                ...
                                                <td><span class="check-price"><%=cart.tongTien()%>.000đ</span></td>
                                            </tr>
                                            <% int khuyenMai = (int) request.getAttribute("khuyenMai");
                                            if(khuyenMai>0){
                                            %>
                                            <tr>
                                                <td colspan="3">Mã giảm giá</td>
                                                <td><span class="check-price"><%=khuyenMai%>%</span></td>
                                            </tr>
                                            <%}%>
                                            <tr>
                                                <td colspan="3"><strong>Thành tiền</strong></td>
                                                <%if(khuyenMai>0){%>
                                                <td><strong><span class="check-price"><%=cart.tongTienKhuyenMai(khuyenMai)%>.000đ</span></strong></td>
                                                <%} else {%>
